...
group=OptionGroup(parser,"FigurePlotting")
group.add_option("--plotfile",help="proteins's name and motif label for figure ")
group.add_option("--name2", help="protein's name and it must be consistent with name defined in procedure MotifCalling ")
parser.add_option_group(group)
...
def str2fa_structMotif():
    ...
    if int(nmax_site) < int(input_Site):
        os.popen("python " + scripts_folder + "/get_fasta_structMotif_meme.py" + "\t" + structAnot + "\t" + output_path + "\t" + protein_name + "\t" + alphabet + "\t" + str(nmax_site)  + "\t" + str(domainMotif_width) + '\t' + mRNA_ref ) # + str(extend) )
        nmax_site=int(input_Site)
        os.popen("python " + scripts_folder + "/get_fasta_structMotif_meme.py" + "\t" + structAnot + "\t" + output_path + "\t" + protein_name + "\t" + alphabet + "\t" + str(nmax_site)  + "\t" + str(domainMotif_width) + '\t' + mRNA_ref )
    else:
	nmax_site=int(input_Site)
        os.popen("python " + scripts_folder + "/get_fasta_structMotif_meme.py" + "\t" + structAnot + "\t" + output_path + "\t" + protein_name + "\t" + alphabet + "\t" + str(nmax_site)  + "\t" + str(domainMotif_width) + '\t' + mRNA_ref )
def fimo_structMotif():
    os.popen("python " + scripts_folder + "/fimo_structMotif_meme.py" + "\t" + structMotif_folder + "\t" + structMotif_fimo_folder + "\t" + struct_path + "\t" + protein_name)
def struct_DataFrame():
    os.popen("python " + scripts_folder + "/structMotif_to_dataframe.py" + "\t" + structMotif_fimo_folder + "\t" + structDataFrame_folder )
def seq_onStruct():
    nmax_site=options.maxsite
    os.popen("python " + scripts_folder + "/seq_for_struct_motif.py" + "\t" + structDataFrame_folder + "\t" + struct_path + "\t" + seq_onStruct_folder + "\t" + str(nmax_site) + "\t" + str(domainMotif_width) )
def seqMotif():
    os.popen("python " + scripts_folder + "/seqMotif_on_structMotif.py" + "\t" + seq_onStruct_folder + "\t" + seqMotif_onStruct_folder)
def seq_DataFrame():
    os.popen("python " + scripts_folder + "/seqMotif_to_dataframe.py" + "\t" + seqMotif_onStruct_folder + "\t" + seqMotif_dataframe_folder )
#
def MotifCalling():
    str2fa_structMotif()
    print "structMotif done\n"
    fimo_structMotif()
    print "fimo_structMotif done\n"
    struct_DataFrame()
    print "struct_DataFrame done\n"
    seq_onStruct()
    print "seq_onStruct done\n"
    seqMotif()
    print "seqMotif done\n"
    seq_DataFrame()
    print "seq_DataFrame done\n"
#
###### Generate Figures#######
if options.plotfile:
    plot_file=options.plotfile
    protein_name=options.name2
    struct_path = input_path + "/" + protein_name + "_allsites"
    structDataFrame_folder = input_path + "/" + protein_name + "_structDataFrame"
    seqMotif_onStruct_folder = input_path + "/" + protein_name + "_seqMotif_onStruct"
    seqMotif_dataframe_folder = input_path + "/" + protein_name + "_seqDataFrame"
def SeqMotifOnDomainMotif_figure():
    os.popen("python " + scripts_folder + "/SeqMotifOnDomainMotif_figure.py" + "\t" + structDataFrame_folder + "\t" + seqMotif_dataframe_folder + "\t" + plot_file + "\t" + struct_path + "\t" + seqMotif_onStruct_folder + '\t' + output_path    )
#
def main():
    if options.forgi: db2ref()
    if options.b1: eCLIPGoing()
    if options.alp: MotifCalling()
    if options.plotfile: SeqMotifOnDomainMotif_figure()
