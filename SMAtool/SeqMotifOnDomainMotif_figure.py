...
#
#
def draw(protein, nprotein, iprotein, label, struct_df, seq_df, fasta_folder, seq_motif):
    domain_trans_df = pandas.DataFrame.from_csv(struct_df + 'binding_sites_on_trans.csv', sep='\t')
    domain_start_df = pandas.DataFrame.from_csv(struct_df + 'motif_start_base.csv', sep='\t')
    seq_trans_df = pandas.DataFrame.from_csv(seq_df + 'binding_sites_on_trans.csv', sep='\t')
    seq_start_df = pandas.DataFrame.from_csv(seq_df + 'motif_start_base.csv', sep='\t')
    hairpin_df = pandas.DataFrame.from_csv(struct_df + 'hairpin.csv', sep='\t')
    stem_df = pandas.DataFrame.from_csv(struct_df + 'stem.csv', sep='\t')
    interior_df = pandas.DataFrame.from_csv(struct_df + 'interior.csv', sep='\t')
    multiloop_df = pandas.DataFrame.from_csv(struct_df + 'multiloop.csv', sep='\t')
#
...
#
    info_df = pandas.DataFrame.from_csv(fasta_folder+protein.split('_')[0]+'_info.csv', sep='\t')
    hh, ss, ii, mm = numpy.zeros(25), numpy.zeros(25), numpy.zeros(25), numpy.zeros(25)
    for xx in s_start.keys():
        x = xx.split('-')[0]
        ...
        hh[s_start[xx]:s_start[xx]+6] = hh[s_start[xx]:s_start[xx]+6] + h_struct[s_start[xx]:s_start[xx]+6]
        ss[s_start[xx]:s_start[xx]+6] = ss[s_start[xx]:s_start[xx]+6] + s_struct[s_start[xx]:s_start[xx]+6]
        ii[s_start[xx]:s_start[xx]+6] = ii[s_start[xx]:s_start[xx]+6] + i_struct[s_start[xx]:s_start[xx]+6]
        mm[s_start[xx]:s_start[xx]+6] = mm[s_start[xx]:s_start[xx]+6] + m_struct[s_start[xx]:s_start[xx]+6]
    n_avail = len(s_start)
...
#
    image_file = seq_motif + protein.split('-')[0] + '/logo' + protein.split('-')[1] + '.png'
...
#
#
name_input = pandas.DataFrame.from_csv(plot_file, sep='\t')   # input list of Motifs
name_proteins = name_input.index.values
name_labels = name_input['label'].values
name_struct_df = structMotif_DataFrame + "/"
name_seq_df = seqMotif_DataFrame + "/"
name_fasta_folder = struct_path + "/"
name_seq_motif = seqMotif_onStruct_folder + "/"
#
#
seaborn.set_context('paper', font_scale=1.8)
fig = plt.figure(figsize=(19, 11))
for i,protein in enumerate(name_proteins):
    print protein
    n_avail = draw(protein, len(name_proteins), i, name_labels[i], name_struct_df, name_seq_df, name_fasta_folder, name_seq_motif)
    print 'number of available sites:', n_avail
fig.savefig( output_path + "/figure.pdf", bbox_inches='tight')
#
