...
#
#[_, halfLength, annotation, minimumNumber,inp, oup] = sys.argv
...
# def normalizedBed(tot):
#     # factor = tot * 5/3/ 1000000.0
#     def generateBed(dic):
#         prev = 0
#         count = 0
#         for key in sorted(dic.keys()):
#             if (count != 0):
#                 yield((prev, key, count))
#             count += dic[key]
#             prev = key
#     return generateBed

# def samToBed(parsedSam, halfLength):
#     chromos, tot = generateChromo(parsedSam, halfLength)
#     return chromos
#     return {chro: map(normalizedBed(tot), vs) for chro, vs in chromos.iteritems()}
#
#
# def toBed(l):
#     def trans(fileH):
#         return samToBed(parseSamFile(fileH), l)
#     return trans
...
def parseAnnotation(fileN):
    ...
    for line in inp:
        ...
        if data[3] == '-':
            ...
            rev = 0
#         name, chromosome, strand, starts, ends
